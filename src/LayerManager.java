...
public class LayerManager { //Layer관리 클래스
	...
	private void MakeList(String pcList){ //들어오는 Layer 이름을 token으로 자름
		...
		for(; tokens.hasMoreElements();){
			...
			AddNode(pNode);//해당 토큰의 노드를 mp_list로 연결
			
		}	
	}

	private _NODE AllocNode(String pcName){ //각 토큰의 노드를 연결
	...
	private void LinkLayer(_NODE pNode){ //계층 간 연결
		...
		while(pNode != null){
			if( pLayer == null)
				...
				if(pNode.token.equals("("))
					...
					switch(cMode){
					case '*':
						Top().SetUpperUnderLayer( pLayer ); //양방향 
						break;
					case '+':
						Top().SetUpperLayer( pLayer ); //윗방향
						break;
					case '-':
						Top().SetUnderLayer( pLayer ); //아래방향
						break;
					}					
				}
			}
